// CAM to LCD - Chromatic Aberration retro video effect //
...
void loop() {
  ...
  for (uint32_t i = 10; i < SCR - 10; i++) {

    uint16_t pR = framebuffer[i];
    uint16_t pG = framebuffer[i-5];
    uint16_t pB = framebuffer[i+5];
    
    framebuffer[i] = (pR & 0xF800) | (pG & 0x07E0) | (pB & 0x001F);
